...
<tr>
<td>Catégories</td> <!-- items qui specifié la liste categories // itemsValues je sait pour chaque categories il contient idcategorie//itemlabes c'est le nom qui va affciher dans la formulaire--> 
<td><f:select path="categorie.idcategorie"  items="${categories}" itemValue="idcategorie" itemLabel="nomCategories"></f:select></td>
<td><f:errors path="categorie.idcategorie" cssClass="errors"></f:errors>
<tr>
<tr>
<td>Description</td>
...
<th>catégorie</th><th>Prix</th><th>Qunatité</th><th>Selected</th>
<th>Photo</th>
</tr>
...
<td>${p.designation}</td>
<td>${p.description}</td>
<td>${p.categorie.nomCategories}</td>
<td>${p.prix}</td>
<td>${p.quantite}</td>
<td>${p.selected}</td>
