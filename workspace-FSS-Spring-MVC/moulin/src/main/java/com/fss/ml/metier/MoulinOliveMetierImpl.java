...
public class MoulinOliveMetierImpl implements MoulinOliveMetier {
	...
	public List<Client> nameClient(String nameClient) {
		// TODO Auto-generated method stub
		return dao.nameClient(nameClient);
	}
...
/*******************Fournisseur***********************/
	...
	public void RetirerMoney(Long idcs, double mt) {
			...
			if (ss.getSolde()>=mt){
			ss.setSolde(ss.getSolde()-mt);
		//	JOptionPane.showMessageDialog(null, "hassen", "caisse",1);
			}else{
...
/********************************Stock******************************/
	...
	public void AjouteHuile(String code, double qtehuile) {
		...
		if(ct.getCapacite()>=(ct.getQuantiteHuile()+qtehuile)){
		ct.setQuantiteHuile(ct.getQuantiteHuile()+qtehuile);}
		else{
			JOptionPane.showMessageDialog(null, " la capacité de cette citerne est insuffisante");
		}
	...
	public void RetirerOlive(String Code, double qteOlive) {
		CuveOlives cuv=(CuveOlives) dao.getStock(Code);
	if(cuv.getQuantiteactOL()-qteOlive>=0){
	System.out.println(cuv.getQuantiteactOL()+"encien quantité");
		cuv.setQuantiteactOL(cuv.getQuantiteactOL()-qteOlive);
		System.out.println(cuv.getQuantiteactOL()+" nouvelle");
	}else{
		JOptionPane.showMessageDialog(null, "la quantité d'olive est indisponible");
	}
	}
	...
	public void AjouterOlive(String Code, double qteOlive) {
		CuveOlives cuv=(CuveOlives) dao.getStock(Code);
		if(cuv.getCapacite()>=(cuv.getQuantiteactOL()+qteOlive)){
		cuv.setQuantiteactOL(cuv.getQuantiteactOL()+qteOlive);
		}else{
			
			JOptionPane.showMessageDialog(null, " la capacité de cet cuve est insuffisant");
...
/*******************BonAchat*************************/
	...
	public BonAchat ajouterBA(BonAchat ba, Long idcaisse, Long CinFourn,String codestock) {
		// TODO Auto-generated method stub
		return dao.ajouterBA(ba, idcaisse, CinFourn, codestock);
	}
	@Override
