...
/**
 * Servlet implementation class SaisieNotesController
 */
@WebServlet("/SaisieNotesController")
public class SaisieNotesController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		{	request.setAttribute("groups", groupeDAO.findAll());	
			...
			{   if(request.getParameter("ds"+nt.getId())!=null){
				nt.setNotepresentielle(Float.parseFloat(0+request.getParameter("pr"+nt.getId())));
			    nt.setNotetp(Float.parseFloat(0+request.getParameter("tp"+nt.getId())));
			    nt.setNoteds(Float.parseFloat(0+request.getParameter("ds"+nt.getId())));
